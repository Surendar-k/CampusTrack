...
import { FaBox, FaPalette, FaCalendarAlt, FaUser } from "react-icons/fa";
  ...
  <div className="min-h-screen bg-gradient-to-br from-purple-800 via-purple-700 to-indigo-600 font-sans p-6">
    ...
    <div className="text-center mb-8">
      ...
      <p className="text-white/90 italic mt-2 text-lg drop-shadow-sm">
        Track lost items with detailed insights
      </p>
    </div>
      ...
      <table className="min-w-full text-left border-collapse">
        ...
        <tbody className="text-white/90 text-sm md:text-base">
          {itemList.map((item) => (
            ...
            >
              <td className="px-4 py-3">{item.lostItemId}</td>
              <td className="px-4 py-3 font-semibold">{item.lostItemName}</td>
              <td className="px-4 py-3">{item.category}</td>
              <td className="px-4 py-3">{item.color}</td>
              <td className="px-4 py-3">{item.brand}</td>
              <td className="px-4 py-3">{item.location}</td>
              <td className="px-4 py-3">{item.lostDate}</td>
